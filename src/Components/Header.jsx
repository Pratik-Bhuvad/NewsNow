...
const Header = () => {
    ...
    return (
        <div className='fixed w-screen'>
            <header className='sticky w-screen h-[10vh] flex justify-between items-center p-4 shadow-md transition-all ease-in-out duration-300 bg-bglight dark:bg-bgdark dark:text-textdark'>
            ...
            <div className={`absolute top-0 right-0 w-2/3 h-screen pb-5 backdrop-blur-[2px] border-l-2 transition-all duration-300 ease-in-out ${sidebar ? 'translate-x-0' : 'translate-x-full'} border-l-gray-200/70 bg-gray-200/20`}>
                ...
                <nav className='flex flex-col space-y-6 py-6'>
                    <NavLink
                        to='/'
                        className={({ isActive }) => `${isActive ? 'text-blue-500 font-bold bg-bgdark' : 'text-gray-700'} p-2 transition-all duration-200 ease-in-out` } >
                        Home
                    </NavLink>
                    <NavLink
                        to='/trending'
